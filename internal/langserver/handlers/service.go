...
func (svc *service) configureSessionDependencies(ctx context.Context, cfgOpts *settings.Options) error {
	...
	modulesFeature, err := fmodules.NewModulesFeature(svc.eventBus, svc.stateStore.DocumentStore, svc.stateStore.JobStore,
		...
		svc.fs, rootModulesFeature, svc.registryClient)
	if err != nil {
		return err
	}
	...
	variablesFeature, err := fvariables.NewVariablesFeature(svc.eventBus, svc.stateStore.JobStore, svc.stateStore.ChangeStore,
